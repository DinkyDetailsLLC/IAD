...
-(id)initWithSize:(CGSize)size lose: (NSInteger)score{
    ...
    if (self = [super initWithSize:size]) {
        ...
        NSLog(@" total score = %d",[gameMechanics getTotalScore]);
        ...
        highScoreLabel.position = ((UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad) ? CGPointMake(CGRectGetMidX(self.frame), CGRectGetMidY(self.frame)) : CGPointMake(CGRectGetMidX(self.frame), CGRectGetMidY(self.frame)));
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if (CGRectContainsPoint(retry.frame, positionInScene)) {
        ...
        [menu runAction:[SKAction sequence:@[[SKAction moveByX:0 y:-2 duration:0.1f], [SKAction moveByX:0 y:2 duration:0.1]]]completion:^{
            [gameMechanics addTotalScore:tepmScore];
